...
| Supported and declared runtime | the runtime is a supported release with no known vulnerability — an LTS or the current widely-used stable, **whichever is clean** — and the project declares it where a tool reads it: `engines` in `package.json`, the `go` directive of `go.mod`, `requires-python` in `pyproject.toml`, `rust-version` in `Cargo.toml`, `ruby` in the `Gemfile` or `.ruby-version`, `require.php` in `composer.json`, or `.tool-versions` for any of them. A Rust `edition` is a language dialect, not a minimum compiler; a version named only in the README is read by people, not by tools | the declaration exists and names a supported version; `node --version` · `go version` against the vendor's support table |
