...
**First calibration (2026-09-17).** Controls: 2 min for a beginner's request and 4 min for a developer's, both a Pokédex CLI. A Fast run for the beginner took 12 min; two Standard runs for the developer planned 43 and 47 boxes and were stopped at 42 min with 28 and 2 of them closed. The multiples above are the target ranges those runs are measured against, **recalibrated at every release** from the harness's own numbers — a figure here that the next round contradicts is rewritten, not defended. **First reading against them (2026-09-17, release commit of 0.7.0):** the beginner's request at `tool`, Standard scale, a 12-box plan, delivered in 23 min against a 2-min control — **11×**, where `tool` promises 2–3× and the rule allows 5×. The run's own report names the always-tier toolchain (formatter, linter, type checker, their config and the gates they generate) as the largest cost, and the program itself as about six of the ten build minutes. What `tool` activates is therefore the first thing the next round recalibrates; the level and the recorded line held, and the plan ceiling was removed for the reason below.
...
**Why there is no task-count ceiling.** Until 0.7.0 the level also carried one — 5 / 12 / 24 / 40 Task List boxes — on the grounds that the measured pace was about one box per minute, so the plan was where the multiple got decided. The pace was an average of a single run and the next two contradicted it (58 and 33 boxes planned, none closed in seventeen minutes), and a checkbox is not a unit of work: `.gitignore` and a whole module are each one. **How many tasks a story has follows from the work, not from its level** — which is also how a work breakdown structure is built, bounding each package's size (the 8/80 heuristic) and never the count, and how agile story splitting works, which prescribes no number at all. The level still decides the plan's *cost* — through what the quality catalog activates and through Phase 3 — and the plan keeps its one threshold on size in bytes ([tasks.md](tasks.md#authoring-ceiling)). The bound that replaced the count is on the unit: a sub-task is one Executor pass, with a `Validation:` command that proves it alone.
